...
//	}
	...
	// checks whether the entity starts with a capital letter		->		Warning
	@Check
	def checkEntityNameStartsWithCapital(Entity entity) {
		if (entity.name.charAt(0).lowerCase)
			warning("Entity name should start with a capital '" + entity.name.charAt(0).toUpperCase + "'",
				EntitiesPackage.eINSTANCE.entity_Name, INVALID_ENTITY_NAME, entity.name);
	}

	// checks whether the attribute starts with a lowercase letter		->		Warning
	@Check
	def checkAttributeNameStartsWithLowercase(Attribute attr) {
		if (attr.name.charAt(0).upperCase)
			warning("Attribute name should start with a lowercase'" + attr.name.charAt(0).toUpperCase + "'",
				EntitiesPackage.eINSTANCE.attribute_Name, INVALID_ATTRIBUTE_NAME, attr.name);
	}
